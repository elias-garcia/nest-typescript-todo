...
describe('ServerRedis', () => {
    ...
    describe('listen', () => {
        ...
        it('should bind "connect" event to handler', () => {
            server.listen(null);
            chai_1.expect(onSpy.getCall(0).args[0]).to.be.equal('connect');
        });
    });
    ...
    describe('handleMessage', () => {
        ...
        it(`should call handler with expected arguments`, () => {
            ...
            server.handleMessage(channel, {}, null);
            chai_1.expect(handler.getCall(0).args[0]).to.eql(data);
        });
    });
    describe('getMessageHandlerCallback', () => {
        let publisherSpy, handler;
        beforeEach(() => {
        ...
        it(`should return function`, () => {
            chai_1.expect(typeof server.getMessageHandlerCallback(null, '')).to.be.eql('function');
        });
        it(`should change order when second parameter is undefined or null`, () => {
            const response = 'test';
            handler(response);
            chai_1.expect(publisherSpy.calledWith({ err: null, response })).to.be.true;
        });
        it(`should call publish with expected message object`, () => {
            const err = 'err';
            const response = 'test';
            handler(err, response);
            chai_1.expect(publisherSpy.calledWith({ err, response })).to.be.true;
        });
    });
